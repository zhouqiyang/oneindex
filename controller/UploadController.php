...
class UploadController{
	...
	function index(){
		if($_POST['upload'] == 1){
			...
			$filename = $_FILES["client"]['name'];
			$localfile = $_FILES["client"]['tmp_name'];
			if (empty($localfile)){
				$message = "文件上传到服务器失败";
				return view::load('upload')->with('message', $message);
			}

			$remotepath = get_absolute_path($_POST['server']);
			$remotefile = config('onedrive_root').$remotepath.$filename;
			$filesize = onedrive::_filesize($localfile);

			if($filesize < 10485760){
				$result = onedrive::upload($remotefile, file_get_contents($localfile));
				if($result){
					$message = "文件上传成功";
				} else {
					$message = "文件上传失败";
				...
				//$tempfile = $_SERVER['DOCUMENT_ROOT'].'\'.$filename;
				//$tempfile = $localfile.'_upload';
				$tempfile = CACHE_PATH.'/upload/'.$filename;
				move_uploaded_file($localfile,$tempfile);
				$task = array(
					'localfile'=>$tempfile,
				...
				$message = "文件<kbd>".$filename."</kbd>已添加到队列";
				$this->task($remotefile);
				/*$request = $this->task_request();
				$request['url'] = substr($request['url'],0,-4).'run';
				fetch::post($request);*/
			}
	...
	function upload_large_file($task){
		...
		if(empty($task['url'])){
			...
			//上传完成
				...
				if ($task['remove_local']){
					//删除本地
					if (file_exists($task['localfile'])){
