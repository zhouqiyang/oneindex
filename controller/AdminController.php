...
class AdminController
{
    ...
    function login()
    {
        if (!empty($_POST['password']) && md5($_POST['password']) == config('password'))
        { /*密文保存密码*/
    ...
    function show()
    {
        ...
        $names = ['stream' => '直接输出(<5M)，走本服务器流量(stream)', 'image' => '图片(image)', 'video' => 'Dplayer 视频(video)', 'video2' => 'Dplayer DASH 视频(video2)/个人版账户不支持', 'video5' => 'html5视频(video5)', 'audio' => '音频播放(audio)', 'code' => '文本/代码(code)', 'doc' => '文档(doc)'];
        $show = config('show');
    ...
    function setpass()
    {
        ...
        {
            if (md5($_POST['old_pass']) == config('password'))
            { /*密文保存*/
    ...
    function install_1()
    {
        ...
        return view::load('install/install_1')->with('title', '系统安装')
            ->with('redirect_uri', $redirect_uri)->with('app_url', $app_url);
    }
